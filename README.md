...
|---|---|---|---|
| `DATABASE_URL` | yes | `postgres://kicca:${POSTGRES_PASSWORD}@postgres:5432/kicca?sslmode=disable` | bare-metal runs point at `localhost` |
...
| `POSTGRES_PASSWORD` | yes | `kicca` | compose postgres credential |
...
Then: point an A/AAAA record for `kicca.apta.works` at the host, publish 443
...
```cron
15 3 * * * docker exec kicca-postgres-1 pg_dump -U kicca kicca | gzip > /var/backups/kicca/kicca-$(date +\%F).sql.gz && find /var/backups/kicca -name 'kicca-*.sql.gz' -mtime +14 -delete
```

(The postgres container name comes from `docker compose ps`; create
`/var/backups/kicca` first. `GH_ENC_KEY` must stay constant across restores or
